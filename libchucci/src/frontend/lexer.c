...
Lexer new_lexer(ChucciCompiler *ctx) {
  Lexer lexer;
  lexer.ctx = ctx;
  lexer.cursor = new_cursor(str_to_sv(kv_top(ctx->options->source_stack)));
  return lexer;
}
...
Token lex_ident(Lexer *lexer) {
  ...
  interned_str ident =
      intern(lexer->ctx->table,
             sv_slice(lexer->cursor.source, pos.id, lexer->cursor.id - pos.id));

#define X(a, b)                                                                \
  if (interned_eq(ident, keywords[a]))                                         \
    return new_token(pos, a, keywords[a].len);
  KEYWORDS(X)
#undef X
...
#define X(a, b, c)                                                             \
  if (str_match_cursor(&lexer->cursor, sv_from_cstr(b)))                       \
    return new_token(pos, a, strlen(b));
  OPERATORS(X)
#undef X
...
#define X(a, b, c)                                                             \
  case c:                                                                      \
    token = new_token(pos, a, strlen(b));                                      \
    advance_cursor(&lexer->cursor);                                            \
    goto end;
    SEPARATORS(X)
#undef X
...
LexerMark mark_lexer(Lexer *lexer) { return mark_cursor(&lexer->cursor); }
