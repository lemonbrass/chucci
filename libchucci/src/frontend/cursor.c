...
void cursor_advance_by(Cursor* cursor, idx_t n) {
  while (n-- > 0) cursor_advance(cursor);
}
...
char cursor_peek_next(Cursor* cursor) {
  if (cursor->source->contents.len < cursor->id + 1) return '\0';
...
bool cursor_match_str(Cursor* cursor, StringView expected) {
  if (expected.len > cursor->source->contents.len - cursor->id) return false;
  Cursor mark = *cursor;
...
bool cursor_match_ch(Cursor* cursor, char expected) {
  if (cursor_peek(cursor) != expected) return false;
  cursor_advance(cursor);
  return true;
}
...
void cursor_dump(Cursor* c) {
  ...
  for (idx_t i = 0; i < (c->col - 1 + prefix); i++) printf(" ");
  printf("^\n");
}
...
CursorMark span_start(Span span) {
  ...
  while (cursor.id < span.start) cursor_advance(&cursor);
...
Span span_from_sv(VMEMArena* arena, StringView sv, idx_t start) {
  assert(start < sv.len);
  File* fileptr = vmarena_alloc(arena, sizeof(File));
  *fileptr = file_new(sv, const_cstr_to_sv("expanded"));
  Span span = {0};
  span.start = start;
  span.len = sv.len - start;
  span.source = fileptr;
  return span;
}
