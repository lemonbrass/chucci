...
string_view get_till_newline_or_eof(Cursor *cursor) {
  ...
  if (sv.len == 0 && sv.cstr == NULL) {
    return sv_slice(cursor->source, cursor->id,
                    cursor->source.len - cursor->id);
  }
  return sv;
}
...
char peek_next(Cursor *cursor) {
  if (cursor->source.len < cursor->id + 1)
    return '\0';
...
bool is_cursor_valid(Cursor *cursor) { return cursor->source.len > cursor->id; }
