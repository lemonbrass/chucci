/*
  ...
  macro_use_fnlike -> first creates a ArgName (interned_str) -> Arg (TokenArray)
  mapping. Then it expands the macro body after replacing all ArgName references
  with their values acc. to the mapping.
  Lastly it recursively checks for more macros before the result is pushed into
  the stream. NOTE: This preprocessor ISNT standard compliant, its a superset of
  a subset, so we will have better macros too, later, but these macros are just
  here as a challenge NOTE: For my short term sanity, im restricting Token
  pasting to just identifiers, will add it later if i wanna get cracked
*/
...
void macro_def_parse_args(Preprocessor2 *pp2, MacroDef *def) {
  ...
  while (true) {
    if (token.kind == SEP_RPAREN)
      ...
      throw_error(pp2->token_source, token,
                  "Unexpected token in macro definition", GLOBAL_CTX(pp2));
...
void macro_def(Preprocessor2 *pp2) {
  MacroDef *def = pp2->ctx->bufdef;

  Token name = expect_token_kind(pp2->token_source, TOK_IDENT, GLOBAL_CTX(pp2));
  def->name = name.ident;
...
void push_fnlike_arg(Preprocessor2 *pp2, TokenArray *arg, MacroCallArgMap *args,
                     interned_str argname) {
...
void macro_use_fnlike_args(Preprocessor2 *pp2, MacroDef *def,
                           MacroCallArgMap *args) {
  expect_token_kind(pp2->token_source, SEP_LPAREN, GLOBAL_CTX(pp2));

  TokenArray *arg = bp_get(GLOBAL_CTX(pp2)->tokenarrays, new_tokenarray);
  size_t argnum = 0;
  ...
  while (true) {
    if (token.kind == SEP_LPAREN) {
      ...
      if (argnum >= kv_size(def->argnames))
        throw_error(pp2->token_source, token,
                    "Excess argumments passed to fnlike macro",
                    GLOBAL_CTX(pp2));
      ...
      if (argnum == 0 && kv_size(def->argnames) == 0) {
        if (arg.n == 0)
          ...
          throw_error(pp2->token_source, token,
                      "Number of passed and needed arguments dont match",
                      GLOBAL_CTX(pp2));
      } else if (argnum + 1 != kv_size(def->argnames))
        throw_error(pp2->token_source, token,
                    "Number of passed and needed arguments dont match",
                    GLOBAL_CTX(pp2));
      push_fnlike_arg(pp2, &arg, args, kv_A(def->argnames, argnum++));
      ...
      throw_error(pp2->token_source, token, "Unexpected EOF", GLOBAL_CTX(pp2));
    else
      kv_push(Token, arg, token);
    token = next_token(pp2->token_source);
  }
  pop_memscope(GLOBAL_CTX(pp2));
}
...
void check_and_substitute_arg(MacroCallArgMap *args, MacroDef *def,
                              TokenArray *expanded, Token token) {
...
bool check_and_join_tokens(Preprocessor2 *pp2, MacroCallArgMap *args,
                           TokenArray *body, TokenArray *expanded,
                           size_t *tokid, Token operand1) {
  if (*tokid + 2 >= kv_size(*body))
    return false;
  ...
  if (operand2.kind != TOK_IDENT)
    return false;

  interned_str pasted =
      concat_intern(GLOBAL_CTX(pp2)->table, interned_to_sv(operand1.ident),
                    interned_to_sv(operand2.ident));
  string pastedstr = {pasted.cstr, pasted.len};
  kv_push(string, GLOBAL_CTX(pp2)->source_stack, pastedstr);
  Lexer lexer = new_lexer(GLOBAL_CTX(pp2));
  Token new = lex_next_token(&lexer); // Token pasting only results in 1 token
  kv_pop(GLOBAL_CTX(pp2)->source_stack);
  *tokid += 2;
...
void stringify(Preprocessor2 *pp2, MacroCallArgMap *args, MacroDef *def,
               TokenArray *expanded, size_t *i) {
  if (*i + 1 >= def->body.n)
    throw_error(pp2->token_source, kv_A(def->body, *i),
                "Stringification operator followed by unexpected EOF",
                GLOBAL_CTX(pp2));
  Token argtok = kv_A(def->body, *i + 1);
  if (argtok.kind != TOK_IDENT)
    throw_error(pp2->token_source, kv_A(def->body, *i + 1),
                "Stringification operator followed by unexpected token",
                GLOBAL_CTX(pp2));
  if (!imap_has(*args, argtok.ident))
    throw_error(
        pp2->token_source, kv_A(def->body, *i + 1),
        "Stringification operator must be followed by a macro parameter",
        GLOBAL_CTX(pp2));
  ...
  if (arg->n == 0) {
    ...
    size_t sum = 0;
    size_t n = 0;
    da_string *buf = &GLOBAL_CTX(pp2)->buf;
    reset_ds(buf);
    push_char_ds(buf, '\"');
    while (n < arg->n)
    ...
    push_char_ds(buf, '\"');
    string str =
        new_str(arena_alloc(GLOBAL_CTX(pp2)->arena, buf->len), buf->len);
...
void macro_use_fnlike_body(Preprocessor2 *pp2, MacroCallArgMap *args,
                           TokenArray *expanded, MacroDef *def) {
  ...
  kv_foreach(Token, *expanded, i, token) {
    if (token.kind == TOK_IDENT &&
        !check_and_join_tokens(pp2, args, expanded, &buf, &i, token))
...
void macro_use_fnlike(Preprocessor2 *pp2, MacroDef *def, TokenArray *out) {
  ...
  push_memscope(GLOBAL_CTX(pp2), scope);
  ...
  // Cyclic macro check
  kv_push(interned_str, GLOBAL_CTX(pp2)->macro_stack, def->name);
  macro_use_fnlike_body(pp2, &args, &buf, def);

  TokenSource src = ts_from_array(buf);
  TokenArray result = recursively_expand(pp2, &src);
  kv_pop(GLOBAL_CTX(pp2)->macro_stack);
...
bool check_cyclic_macros(Preprocessor2 *pp2, interned_str name) {
  kv_foreach(interned_str, GLOBAL_CTX(pp2)->macro_stack, i, macro) {
    if (interned_eq(macro, name))
      return true;
  }
  return false;
}
...
void macro_use(Preprocessor2 *pp2, Token *token, TokenArray *out) {
  MacroDef *def = imap_get(GLOBAL_CTX(pp2)->macros, token->ident);
  if (!def)
    return;
